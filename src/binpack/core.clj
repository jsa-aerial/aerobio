...
(extend-protocol Packable
  ...
  (pack [s]
    (cond-let [bytes (.getBytes s)
              ...
              (<= len 0x0f)       (ubytes (cons (bit-or 2r10110000 len) bytes))
              (<= len 0xffff)     (ubytes (concat [0xd8] (short->bytes len) bytes))
              (<= len 0xffffffff) (ubytes (concat [0xd9] (int->bytes len) bytes))
              ))
...
(defn- pack-bytes [bytes]
  (cond-let [len (count bytes)]
            ;; BinaryPack uses 0xda and 0xdb and has no byte size len,
            ;; but has a ridiculous type/size byte
            ...
            (<= len 0x0f)       (ubytes (concat [(+ 0xa0 len)] bytes))
            (<= len 0xffff)     (ubytes (concat [0xda] (short->bytes len) bytes))
            (<= len 0xffffffff) (ubytes (concat [0xdb] (int->bytes len) bytes))
            ))
...
(defn- unpack-stream
  ...
  ([stream flags]
   (cond-let [b (next-byte stream)
             ...
             ;; BinaryPack uses 0xb0 for type/size byte (not 0xa0) and
             ;; uses 0xd8 and 0xd9 and has no byte size len
             ;;(= (bit-and 2r11100000 b) 2r10100000) (next-string (bit-and 2r11111 b) stream)
             ...
             (= (bit-and 2r11110000 ub) 2r10110000) (next-string (bit-and 2r1111 ub) stream)
             (= ub 0xd8) (next-string (unsigned (next-short stream)) stream)
             (= ub 0xd9) (next-string (unsigned (next-int stream)) stream)
             ...
             (= (bit-and 2r11110000 ub) 2r10100000) (next-bytes (unsigned (bit-and 2r1111 ub)) stream)
             (= ub 0xda) (next-bytes (unsigned (next-short stream)) stream)
             (= ub 0xdb) (next-bytes (unsigned (next-int stream)) stream)
