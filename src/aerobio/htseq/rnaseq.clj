...
(defn bcfreqs [fq]
  (letio [kw (->> fq fs/basename (str/split #"\.") first
                  (str/split #"_") (drop 2) first keyword)
          inf (->> fq clojure.java.io/input-stream
                   java.util.zip.GZIPInputStream.)
          lines (io/read-lines inf)]
    [kw
     (reduce (fn[[bcM ntM] l]
               (let [bc (str/substring (second l) 0 7)]
                 [(assoc bcM bc (inc (get bcM bc 0)))
                  (merge-with + ntM (p/freqn 1 (second l)))]))
             [{} {}] (partition-all 4 lines))]))
...
(defn bcfreqs-fold
  ...
  (letio [kw (->> fq fs/basename
          ...
          partition (long (/ rec-chunk-size 4 100))]
    [kw (loop [lines lines
               [bcM ntM] [{} {}]]
          (let [chunk (take rec-chunk-size lines)]
            (if (not (seq chunk))
              [bcM ntM]
              (let [[bcm ntm]
                    (r/fold
                     ...
                     (coll/partitionv-all 4 chunk))]
                (recur (drop rec-chunk-size lines)
                       [(merge-with + bcM bcm) (merge-with + ntM ntm)])))))]))
...
(defn editdist-barcodes [s barcodes]
  (map #(it/levenshtein s %) barcodes))

(defn barcodes-edist-1-seqs
  [bcM barcodes]
  (let [all (map (fn[[x _]]
                   (vector x (keep #(when (= 1 (it/levenshtein x %)) %)
                                   barcodes)))
...
(defn open-streaming-gzip [fspec io]
  (if (= io :in)
...
(defn write-recs-to-file [ot recs]
  (doseq [[id sq aux qc] recs]
    (.write ot (str id "\n"))
    (.write ot (str sq "\n"))
    (.write ot (str aux "\n"))
    (.write ot (str qc "\n"))))
    ...
    (try
      (loop [lines lines]
        (let [chunk (take rec-chunk-size lines)]
          (if (not (seq lines))
            :done
            (let [recs (partition-all 4 chunk)
                  recs (r/fold
                        partition
                        (fn([] [])
                          ([V v]
...
(defn get-phase-1-args [eid repname & {:keys [repk]}]
  (let [fqs (cljstr/join "," (get-replicate-fqzs eid repname repk))
        refnm (replicate-name->strain-name eid repname)
        btindex (fs/join (get-exp-info eid :index) refnm)
        otbam (fs/join (get-exp-info eid repk :bams) (str repname ".bam"))
        otbai (str otbam ".bai")
        refgtf (fs/join (get-exp-info eid :refs)
                        (str refnm ".gtf"))
        cuffot (fs/join (get-exp-info eid repk :cuffs) repname)]
...
(defn get-phase-2-dirs [eid nm repk]
  (let [cuffbase  (fs/join (get-exp-info eid repk :cuffs))
        charts    (fs/join (get-exp-info eid repk :charts))
        charts    (fs/join charts nm)
        asms      (fs/join (get-exp-info eid repk :asms))
        diffs     (fs/join (get-exp-info eid repk :diffs))
        bams      (fs/join (get-exp-info eid repk :bams))
...
(defn get-phase-2-args [eid nm & {:keys [repk]}]
  (let [[cuffbase
        ...
        replicates (if repk
                     ((get-exp-info eid :replicate-names) nm)
                     (->> (get-exp-info eid :sample-names)
                          (filter (fn[n]
        ...
        refnm     ((get-exp-info eid :ncbi-sample-xref) strain)
        merge-gtf (fs/join merge-dir "merged.gtf")
        asm-txt   (fs/join asms (str nm "-assembly.txt"))
        refgtf    (fs/join (get-exp-info eid :refs) (str refnm ".gtf"))
        reffna    (fs/join (get-exp-info eid :refs) (str refnm ".fna"))
        comps (if repk
                repstr
                (->> "ComparisonSheet.csv"
...
(defn run-rnaseq-phase-2
  ...
  (let [rnaseq-phase2-job-template template
        sampnms (get-exp-info eid :sample-names)
        strains (get-exp-info eid :strains)
